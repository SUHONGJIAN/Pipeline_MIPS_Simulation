...
int main()
{
    ...
    while (1) {
        ...
        {                       
            ...
            {
                if (((state.EX.is_I_type == 0) && (state.EX.wrt_enable == 1)) || (1 == state.EX.wrt_mem))   //当前指令addu, subu, sw
                {
        ...
        /* --------------------- ID stage --------------------- */   //对newState.EX进行更新(除了Read_data1, Read_data2, Imm, Rs, Rt之外, 其他需要按opcode分类讨论); 判断并执行branch跳转; 判断[1 distance]produce-MEM(lw), consume-EX(addu,subu,sw仅rs)的stall, 为了后面的MEM-EX forwarding
        if (state.ID.nop == 0)
        {
