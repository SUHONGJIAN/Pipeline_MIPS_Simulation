...
int main()
{
    ...
    while (1) {
        ...
        if (0 == state.ID.nop)
        {
            ...
            else if (opcode == "000100")	//Branch On Equal if(R[rs]==R[rt]) PC=PC+4+BrachAddr
            {
            ...
            else if (opcode == "000010") {         // 支持jump instruction.
            	newState.EX.INS = "j";
                newState.EX.Wrt_reg_addr = 0;
                newState.EX.is_I_type = 0;
                newState.EX.alu_op = 1;
            	...
            	newState.IF.PC = bitset<32> (state.IF.PC.to_string().substr(0, 4) + Instruction.to_string().substr(25, 26) + "00");
        ...
        cout << "IPC: " << (5 - (state.IF.nop + state.ID.nop + state.EX.nop + state.MEM.nop + state.WB.nop)) / 5 << endl;

        cout << "DataMmeUtilization: " << (state.MEM.wrt_mem | state.MEM.rd_mem) << endl;
    }
